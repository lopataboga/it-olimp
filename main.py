...
class GardenCommunication():
    ...
    def windowOpen(self, state_value):
        # response = requests.patch("https://dt.miet.ru/ppo_it/api/fork_drive/", data ={"state": state_value})
        response = requests.request('patch', "https://dt.miet.ru/ppo_it/api/fork_drive/", data ={"state": state_value}, headers={"X-Auth-Token": atoken})
        return response.status_code

    def waterGarden(self, device_id, state_value):
        response = requests.request('patch', "https://dt.miet.ru/ppo_it/api/watering/", data ={"id": device_id, "state": state_value}, headers={"X-Auth-Token": atoken})
        return response.status_code

    def globalWaterGarden(self, state_value):
        response = requests.patch("https://dt.miet.ru/ppo_it/api/total_hum/", data={"state": state_value})
        return response.status_code
